...
class Bimanual_InterGripperProprio_DeltaActions(DataTransformFn):
    ...
    def __call__(self, data):
        ...
        # valid action format is [left_6d_rot, left_ee_ik_target_handle_position, left_gripper_pos, right_6d_rot, right_ee_ik_target_handle_position, right_gripper_pos]
        actions, state = data["action"], data["proprio"]

        # import viser # viser debug
        # viser_server = viser.ViserServer()
        
        left_t0_in_world = vtf.SE3.from_rotation_and_translation(
            vtf.SO3(wxyz=rot_6d_to_quat(np.asarray(state[:, :6]))), np.asarray(state[:, 6:9])
        )
        ...
        #     viser_server.scene.add_frame(f"left_t0_in_world", position=left_t0_in_world.wxyz_xyz[i, -3:], wxyz=left_t0_in_world.wxyz_xyz[i, :4])
        #     viser_server.scene.add_frame(f"right_t0_in_world", position=right_t0_in_world.wxyz_xyz[i, -3:], wxyz=right_t0_in_world.wxyz_xyz[i, :4])
