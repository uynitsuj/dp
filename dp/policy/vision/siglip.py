...
class SpatialSoftmaxHead(nn.Module):
    def __init__(self, in_channels: int, num_kp: int = 32, temperature: float = 1.0):
        ...
        self.to_kp = nn.Conv2d(in_channels, num_kp, kernel_size=1, bias=True)  # <- no Lazy
...
# --------------------- SigLIP conv-like adapter ---------------------

class SigLIPBackboneConvAdapter(nn.Module):
    def __init__(self, model_name: str, target_size, freeze: bool, normalize_images: bool = True):
        ...
        # ---- load VISION-ONLY backbone ----
        self.backbone = None
        try:
            # Preferred when your transformers has SiglipVisionModel
            from transformers import SiglipVisionModel  # type: ignore
            self.backbone = SiglipVisionModel.from_pretrained(model_name)
        except Exception:
            try:
                # Many SigLIP repos store the vision weights in a subfolder
                self.backbone = AutoModel.from_pretrained(
                    model_name, subfolder="vision_model", trust_remote_code=True
                )
            except Exception:
                # Last resort: load full model and take its vision_model
                full = AutoModel.from_pretrained(model_name, trust_remote_code=True)
                if hasattr(full, "vision_model"):
                    self.backbone = full.vision_model
                else:
                    raise RuntimeError("Could not obtain SigLIP vision backbone from the given model.")

        # Pull patch size (and default image size, if you want to assert target_size)
        cfg = getattr(self.backbone, "config", None)
        vcfg = getattr(cfg, "vision_config", cfg) if cfg is not None else None
        self.patch_size = int(getattr(vcfg, "patch_size", 14))
        self.out_channels = int(getattr(vcfg, "hidden_size", 768))
        # Optional: assert target_size matches the model’s native image size
        # native = int(getattr(vcfg, "image_size", target_size[0]))
        # assert target_size[0] == native and target_size[1] == native

        mean = getattr(self.processor, "image_mean", [0.48145466, 0.4578275, 0.40821073])
        std  = getattr(self.processor, "image_std",  [0.26862954, 0.26130258, 0.27577711])
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # x: (B, 3, H, W); assume your upstream normalizer already did what you want
        if x.dtype not in (torch.float32, torch.float16, torch.bfloat16):
            x = x.float()

        # still resize to keep token grid valid for ViT
        if (x.shape[-2], x.shape[-1]) != self.target_size:
            x = torch.nn.functional.interpolate(x, size=self.target_size, mode="bilinear", align_corners=False)

        if self.normalize_images:
            # Only apply SigLIP normalization if requested
            if x.max() > 1.0:
                x = x / 255.0
            x = (x - self.mean.to(x.device, x.dtype)) / self.std.to(x.device, x.dtype)

        out = self.backbone(pixel_values=x).last_hidden_state  # (B, N, D)
        H, W = self.target_size
        gh, gw = H // self.patch_size, W // self.patch_size
        B, N, D = out.shape
        assert N == gh * gw, f"Tokens {N} != {gh}*{gw}"
        return out.transpose(1, 2).contiguous().view(B, D, gh, gw)

# --------------------- image branch (crop -> SigLIP -> softmax) ---------------------

class SigLIPImageBranch(nn.Module):
    def __init__(
        self,
        model_name: str,
        ...
        self.backbone = SigLIPBackboneConvAdapter(model_name, target_size, freeze=freeze_backbone, normalize_images=normalize_images)
    ...
    #     self.pool = SpatialSoftmaxHead(in_channels=self.backbone.out_channels, num_kp=num_kp)

    # def forward(self, x: torch.Tensor) -> torch.Tensor:
    #     # x: (B, 3, H, W) -> crop -> backbone -> (B, D, gh, gw) -> (B, 2*num_kp)
    #     center = (not self.training) or self.eval_fixed_crop
    #     x = _rand_or_center_crop(x, self.crop_h, self.crop_w, center=center)
    #     feat = self.backbone(x)
    #     return self.pool(feat)
...
class SigLIPObsEncoder(nn.Module):
    """
    Dict-in encoder compatible with your current call site.
    RGB keys -> Crop -> SigLIP -> SpatialSoftmax(2K)
    ...
    def __init__(
        ...
        # per-RGB-key branches (no weight sharing by default, mirroring robomimic)
        self.image_branches = nn.ModuleDict()
        for k in self.rgb_keys:
            self.image_branches[k] = SigLIPImageBranch(
    ...
    # keep robomimic-like API your code expects
...
if __name__ == "__main__":
    ...
    siglip_obs_encoder = SigLIPObsEncoder(
        obs_key_shapes=obs_key_shapes,
        model_name = "google/siglip-base-patch16-224",
        crop_size=(200, 200),          # keep your crop behavior
        target_size=(224, 224),        # keep resize for ViT tokens
        num_kp=32,
        freeze_backbone=False,
        eval_fixed_crop=False,
        normalize_images=False,        # <<< skip SigLIP mean/std; you already normalized
    )
    B_flat = 32  # e.g., batch_size * n_obs_steps
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    siglip_obs_encoder.to(device).train()  # or .eval()
    ...
    # your existing code builds `siglip_obs_encoder` & `dummy_obs` here...

    torch.cuda.synchronize(device); _mem(device, "[after build] ")

    # ---- forward ----
    out = siglip_obs_encoder(dummy_obs)
    ...
    # one grad check
    for n, p in siglip_obs_encoder.named_parameters():
