...
def file_is_left(file_path):
    data = zarr.load(file_path) 
    data = np.any(np.array(data), axis=0)
    left, _ = data
    return left

class SequenceDataset(Dataset):
    # action_key : str = "action/cartesian_pose" # [LEFT ARM] w, x, y, z, -- x, y, z + [RIGHT ARM] w, x, y, z -- x, y, z
    proprio_key : str = "ee_poses" # [LEFT ARM] w, x, y, z, -- x, y, z + [RIGHT ARM] w, x, y, z -- x, y, z
    joint_key : str = "joint_angles" # for getting gripper information # (n, 16) dtype('float64')
    traj_start_idx : int = 0
    gripper_width :  int = 0.024
    ...
    def __init__(
        ...
        if dataset_config.data_subsample_ratio > 0 or dataset_config.data_subsample_num_traj > 0:
            all_fils = os.listdir(self.dataset_root)
            envs_indices = [int(f.split("_")[1]) for f in all_fils if "env" in f]
            unique_envs = np.unique(envs_indices)
            rng = np.random.default_rng(seed=shared_config.seed)
            rng.shuffle(unique_envs)
        
            subsampled_envs = unique_envs
            subsampled_files = [f for f in all_fils if int(f.split("_")[1]) in subsampled_envs]
            subsampled_folders = [os.path.join(self.dataset_root, f) for f in subsampled_files]
            self.h5_files = list(chain.from_iterable([glob(os.path.join(folder, '**', '*.h5'), recursive=True) for folder in subsampled_folders]))
            if dataset_config.data_subsample_num_traj > 0:
                self.h5_files = self.h5_files[:int(dataset_config.data_subsample_num_traj / dataset_config.train_split)]
            elif dataset_config.data_subsample_ratio > 0:
                self.h5_files = self.h5_files[:int(dataset_config.data_subsample_ratio * len(self.h5_files) / dataset_config.train_split)]
            print("Length of data after subsampling: ", len(self.h5_files))
        else:
        ...
        # balance data that only uses left or right arm 
        if dataset_config.balance_data:
            print("Length of data before balancing: ", len(self.h5_files))
            self.h5_files = self.balance_left_right()
            print("Length of data after balancing: ", len(self.h5_files))
        ...
        action_stats_path = os.path.join(logging_config.output_dir, "action_statistics.json")
        if os.path.exists(action_stats_path):
            dataset_config.action_statistics = action_stats_path
        if dataset_config.action_statistics is not None:
            ...
            min_action, max_action = self.calculate_dataset_statistics(action_stats_path)
        self.stats = {
        ...
        new_start_end = []
        for h5_file, start, end in self.start_end:
            for i in range(start, end - self.subsample_steps * self.num_pred_steps, self.subsample_steps):
                new_start_end.append((h5_file, i, i + self.subsample_steps * self.num_pred_steps))
    ...
    def balance_left_right(self):
        gripper_cmd_paths = [f.replace('.h5', '_gripper_cmd.zarr') for f in self.h5_files]

        # make sure gripper cmd file exists, otherwise do not apply any changes 
        if not all([os.path.exists(f) for f in gripper_cmd_paths]):
            print("Gripper command files do not exist, not balancing left and right data")
            return self.h5_files
        left_files = [file_path for file_path in gripper_cmd_paths if file_is_left(file_path)]
        right_files = [file_path for file_path in gripper_cmd_paths if file_path not in left_files]
        rng = np.random.default_rng(seed=self.seed)
        # make sure left and right files have equal length 
        min_length = min(len(left_files), len(right_files))
        # sample left and right files without replacement
        left_files = rng.choice(left_files, min_length, replace=False)
        right_files = rng.choice(right_files, min_length, replace=False)
        # combine left and right files
        fps = np.concatenate((left_files, right_files)).tolist()
        # get original h5 files
        h5_files = [f.replace('_gripper_cmd.zarr', '.h5') for f in fps]
    ...
    def calculate_dataset_statistics(
        ...
        for h5_file in tqdm(self.h5_files):
            ...
            left_proprio, right_proprio = self.helper_load_proprio(h5_file, self.traj_start_idx, self.get_traj_length(h5_file) - self.subsample_steps, noisy = (self.proprio_noise > 0.0))
    ...
    def get_h5_length(
        ...
        return len(h5_file[self.proprio_key][:])
    ...
    def __getitem__(self, idx : int):
        ...
        left_proprio, right_proprio = self.helper_load_proprio(h5_file, start, start + 1, noisy = (self.proprio_noise > 0.0))
    ...
    def helper_load_action(self, h5_file_path : str, start : int, end : int):
        ...
        action_fp = h5_file_path.replace(".h5", "_proprio.zarr")
        gripper_fp = h5_file_path.replace(".h5", "_gripper_cmd.zarr")
        ...
        # get gripper data
        if os.path.exists(gripper_fp):
            gripper = zarr.load(gripper_fp)[indices]
            left_g, right_g = gripper[:, 0][:, None], gripper[:, 1][:, None] # Binary invert since data is recorded with 1.0 == Close where for model we want 1.0 * gripper_width == Open
            left_g, right_g = left_g.astype(float), right_g.astype(float)
        else:
            joint_data = zarr.load(joint_fp)[indices]
            left_g, right_g = joint_data[:, -2][:, None], joint_data[:, -1][:, None]
        ...
        left = np.concatenate([
            left[:, :3],
            quat_to_rot_6d(left[:, 3:]), 
            left_g * self.scale_left_gripper
        ], axis=1)
        right = np.concatenate([
            right[:, :3],
            quat_to_rot_6d(right[:, 3:]), 
            right_g * self.scale_right_gripper
        ], axis=1)
        return left, right
    ...
    def randomize(self, transform : np.ndarray):
        ...
        # transform is xyz, wxyz
        t, rot = transform[:, :3], transform[:, 3:]
        ...
        tr = np.concatenate([t, rot], axis=1)
        return tr
    ...
    def helper_load_proprio(self, h5_file_path : str, start : int, end : int, noisy : bool = True):
        ...
        if os.path.exists(gripper_fp):
            gripper = zarr.load(gripper_fp)[indices]
            left_g, right_g = gripper[:, 0][:, None], gripper[:, 1][:, None]
            left_g, right_g = left_g.astype(float), right_g.astype(float)
        else:
            joint_data = zarr.load(joint_fp)[indices]
            left_g, right_g = joint_data[:, -2][:, None], joint_data[:, -1][:, None]
    ...
    def helper_load_camera(self, h5_file_path : str, start : int, end : int):
        ...
        # import pdb; pdb.set_trace()
        parent_dir = os.path.dirname(os.path.dirname(h5_file_path))

        image_left_path = os.path.join(parent_dir, f"camera_1/rgb/{start:04d}.jpg")
        image_right_path = os.path.join(parent_dir, f"camera_0/rgb/{start:04d}.jpg")
        # image_left_path = h5_file_path.replace(".h5", f"_left/{start:04d}.jpg")
        # image_right_path = h5_file_path.replace(".h5", f"_right/{start:04d}.jpg")
        camera_observations = []
